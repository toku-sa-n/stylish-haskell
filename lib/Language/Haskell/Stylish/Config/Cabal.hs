...
findCabalFile verbose = do
  ...
  potentialCabalFile <-
    filter isRight <$> traverse Cabal.findPackageDesc potentialProjectRoots
...
readDefaultLanguageExtensions ::
     Verbose -> FilePath -> IO [Language.KnownExtension]
readDefaultLanguageExtensions verbose cabalFile = do
  ...
  let library :: [Cabal.Library]
      library =
        maybeToList $
        fst . Cabal.ignoreConditions <$> Cabal.condLibrary packageDescription
      subLibraries :: [Cabal.Library]
      subLibraries =
        fst . Cabal.ignoreConditions . snd <$>
      ...
      gatherBuildInfos =
        map Cabal.libBuildInfo library <>
      ...
      defaultExtensions =
        map fromEnabled . filter isEnabled $
        concatMap Cabal.defaultExtensions gatherBuildInfos
        where
          isEnabled (Language.EnableExtension _) = True
          isEnabled _                            = False
          fromEnabled (Language.EnableExtension x) = x
          fromEnabled x =
            error $
            "Language.Haskell.Stylish.Config.readLanguageExtensions: " <>
