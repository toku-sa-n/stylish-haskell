...
newtype Comments =
  Comments [GHC.RealLocated GHC.EpaComment]
...
groupByLine f = go [] Nothing
  where
    ...
    go acc mbCurrentLine (x:xs) =
      let lStart = GHC.srcSpanStartLine (f x)
          lEnd = GHC.srcSpanEndLine (f x)
       in case mbCurrentLine of
            Just lPrevEnd
              | lPrevEnd + 1 < lStart -> ne acc ++ go [x] (Just lEnd) xs
  ...
  where
    newImportNames =
      case (ideclHiding i0, ideclHiding i1) of
        (Just (b, L p imps0), Just (_, L _ imps1)) ->
          Just (b, L p (imps0 `merge` imps1))
        (Nothing, Nothing) -> Nothing
        (Just x, Nothing) -> Just x
        (Nothing, Just x) -> Just x
    merge xs ys = L.nubBy ((==) `on` showOutputable) (xs ++ ys)
...
moduleLanguagePragmas = mapMaybe prag . epAnnComments . GHC.hsmodAnn . GHC.unLoc
  where
    prag :: GHC.LEpaComment -> Maybe (GHC.RealSrcSpan, NonEmpty String)
    prag comment =
      case GHC.ac_tok (GHC.unLoc comment) of
        GHC.EpaBlockComment str
          | lang:p1:ps <- tokenize str
          , map toLower lang == "language" ->
