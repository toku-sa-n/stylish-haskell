...
parseModule externalExts0 fp string
    -- Parse extensions.
 = do
  externalExts1 <-
    for externalExts0 $ \s ->
      case GHCEx.readExtension s of
  ...
  let fileOptions =
        ...
        GHC.getOptions
          ...
          (fromMaybe "-" fp)
      fileExtensions =
        mapMaybe (stripPrefix "-X" >=> GHCEx.readExtension) fileOptions
    -- Set further dynflags.
  let dynFlags1 =
        foldl' turnOn dynFlags0 fileExtensions `GHC.gopt_set`
        GHC.Opt_KeepRawTokenStream
    -- Possibly strip CPP.
  let removeCpp s =
        if GHC.xopt LangExt.Cpp dynFlags1
          then unCpp s
          else s
