  ...
  case decode1Strict bytes of
    ...
    Right config -> do
      cabalLanguageExtensions <-
        if configCabal config
          then map show <$> Cabal.findLanguageExtensions verbose
          else pure []
      return $
        config
          { configLanguageExtensions =
              nub $ configLanguageExtensions config ++ cabalLanguageExtensions
          }

--------------------------------------------------------------------------------
parseConfig :: A.Value -> A.Parser Config
parseConfig (A.Object o)
 ...
 = do
  ...
  where
    newlines = [("native", IO.nativeNewline), ("lf", IO.LF), ("crlf", IO.CRLF)]
    exitCodes =
...
parseSteps config val = do
  ...
  forM (M.toList map') $ \(k, v) ->
    case (M.lookup k catalog, v) of
...
parseEnum strs _ (Just k) =
  case lookup k strs of
    Just v -> return v
    Nothing ->
      fail $
      "Unknown option: " ++
      k ++ ", should be one of: " ++ intercalate ", " (map fst strs)

--------------------------------------------------------------------------------
parseModuleHeader :: Config -> A.Object -> A.Parser Step
parseModuleHeader config o =
  fmap (ModuleHeader.step columns) $
  ModuleHeader.Config <$> (o A..:? "indent" A..!= ModuleHeader.indent def) <*>
  ...
  (o A..:? "open_bracket" >>=
   parseEnum openBracket (ModuleHeader.openBracket def))
  where
...
parseRecords c o =
  Data.step <$>
  (Data.Config <$> (o A..: "equals" >>= parseIndent) <*>
  ...
  where
    configMaxColumns = maybe Data.NoMaxColumns Data.MaxColumns (configColumns c)
...
parseImports config o =
  fmap (Imports.step columns) $
  Imports.Options <$>
  ...
  (o A..:? "list_padding" >>=
   maybe (pure $ def Imports.listPadding) parseListPadding) <*>
  ...
  where
    ...
    emptyListAligns =
      [("inherit", Imports.Inherit), ("right_after", Imports.RightAfter)]
    parseListPadding =
      \case
        A.String "module_name" -> pure Imports.LPModuleName
        A.Number n
          | n >= 1 -> pure $ Imports.LPConstant (truncate n)
...
parseTabs _ o = Tabs.step <$> o A..:? "spaces" A..!= 8
...
parseUnicodeSyntax _ o =
  UnicodeSyntax.step <$> o A..:? "add_language_pragma" A..!= True <*>
  mkLanguage o
