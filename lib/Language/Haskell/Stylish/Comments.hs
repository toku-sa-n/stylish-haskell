...
instance GHC.Outputable a => Show (CommentGroup a) where
  show CommentGroup {..} =
    ...
    ") (" ++
    showOutputable cgItems ++ ") (" ++ showOutputable cgFollowing ++ "))"
...
commentGroups getSpan allItems allComments =
  ...
  where
    ...
    work ::
      ...
      -> [CommentGroup a]
    work mbCurrent items comments =
      case takeNext items comments of
        Nothing -> maybeToList mbCurrent
        Just (b, next, items', comments') ->
          let (flush, current) =
                case mbCurrent of
                  Just c
                    | adjacent (cgBlock c) b
                    , nextThingItem next
              ...
              current' =
                case next of
                  ...
                  NextComment c
                    | null (cgItems current) ->
                      current {cgPrior = cgPrior current <> [c]}
                    | otherwise ->
                      current {cgFollowing = cgFollowing current <> [c]}
                  NextItemWithComment i c ->
                    current {cgItems = cgItems current <> [(i, Just c)]}
           in flush ++ work (Just current') items' comments'
...
takeNext ::
  ...
  -> Maybe ( LineBlock
           ...
           , [(LineBlock, GHC.LEpaComment)])
takeNext [] [] = Nothing
takeNext [] ((cb, c):comments) = Just (cb, NextComment c, [], comments)
takeNext ((ib, i):items) [] = Just (ib, NextItem i, items, [])
takeNext ((ib, i):items) ((cb, c):comments)
  ...
  | otherwise = Just (cb, NextComment c, (ib, i) : items, comments)
...
commentGroupHasComments CommentGroup {..} =
  not $ null cgPrior && all (isNothing . snd) cgItems && null cgFollowing

--------------------------------------------------------------------------------
commentGroupSort :: (a -> a -> Ordering) -> CommentGroup a -> CommentGroup a
commentGroupSort cmp cg = cg {cgItems = sortBy (cmp `on` fst) (cgItems cg)}
