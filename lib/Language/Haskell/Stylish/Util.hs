...
wrap maxWidth leading ind = wrap' leading
  where
    wrap' ss [] = [ss]
    wrap' ss (str:strs)
      | overflows ss str = ss : wrapRest maxWidth ind (str : strs)
...
--------------------------------------------------------------------------------
wrapMaybe ::
     Maybe Int -- ^ Maximum line width (maybe)
  -> String -- ^ Leading string
  -> Int -- ^ Indentation
  -> [String] -- ^ Strings to add/wrap
  -> Lines -- ^ Resulting lines
...
getGuards (Hs.Match _ _ _ grhss) =
  let lgrhs = getLocGRHS grhss -- []
      grhs = map GHC.unLoc lgrhs
   in concatMap getGuardLStmts grhs

getLocGRHS ::
     Hs.GRHSs Hs.GhcPs (Hs.LHsExpr Hs.GhcPs)
  -> [Hs.LGRHS Hs.GhcPs (Hs.LHsExpr Hs.GhcPs)]
getLocGRHS (Hs.GRHSs _ guardeds _) = guardeds

getGuardLStmts ::
     Hs.GRHS Hs.GhcPs (Hs.LHsExpr Hs.GhcPs) -> [Hs.GuardLStmt Hs.GhcPs]
