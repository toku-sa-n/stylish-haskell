...
compactPragmas lg columns pragmas' =
  wrapMaybe columns ("{-# " ++ lg) 13 $
...
compactLinePragmas lg columns align pragmas' = map (wrapLanguage . pad) prags
  where
    ...
    prags =
      map truncateComma $
      wrapMaybe maxWidth "" 1 $ map (++ ",") (init pragmas') ++ [last pragmas']
...
--------------------------------------------------------------------------------
prettyPragmas ::
     String -> Maybe Int -> Int -> Bool -> Style -> [String] -> Lines
...
filterRedundant :: (String -> Bool) -> [(l, NonEmpty String)] -> [(l, [String])]
filterRedundant isRedundant' =
  snd . foldr filterRedundant' (S.empty, []) . fmap (fmap toList)
  where
...
--------------------------------------------------------------------------------
step' ::
     Maybe Int -> Style -> Bool -> Bool -> String -> Lines -> Module -> Lines
step' columns style align removeRedundant lngPrefix ls m
  ...
  where
    ...
    groupAdjacent' = fmap turnSndBackToNel . groupAdjacent . fmap (fmap toList)
      where
    ...
    groups =
      [ (b, pgs)
      | (b, pgs) <- groupAdjacent' (convertFstToBlock languagePragmas)
      ]
    changes =
      mconcat
        [ Editor.changeLines
          b
          (const $ prettyPragmas lngPrefix columns longest align style pg)
...
isRedundantBangPatterns modul =
  ...
  where
    ...
    getMatchStrict (GHC.Match _ ctx _ _) =
      case ctx of
