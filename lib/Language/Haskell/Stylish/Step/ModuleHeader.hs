...
printModuleHeader maxCols conf ls lmodul =
  let modul = GHC.unLoc lmodul
      ...
      endLine =
        fromMaybe 1 $
        whereLine <|>
        (do loc <- GHC.getLocA <$> GHC.hsmodExports modul
      ...
      exportGroups =
        case GHC.hsmodExports modul of
          Nothing -> Nothing
          Just lexports ->
            Just $
            doSort $
      ...
      changes =
        Editor.changeLines
          ...
          (const printedModuleHeader)
   in Editor.apply changes ls
  where
    doSort =
      if sort conf
        then fmap (commentGroupSort compareLIE)
        else id
...
printSingleLineExportList :: Config -> [GHC.LIE GHC.GhcPs] -> P ()
...
printMultiLineExportList :: Config -> [CommentGroup (GHC.LIE GHC.GhcPs)] -> P ()
  ...
  where
    printExport (CommentGroup {..}, firstGroup, _lastGroup) = do
      ...
      forM_ (flagEnds cgItems) $ \((export, mbComment), start, _end) -> do
        if firstGroup && start
          then unless (null cgPrior) $ space >> space
          else comma >> space
