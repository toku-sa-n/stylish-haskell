...
groupAlign ::
     Align -> [Alignable GHC.RealSrcSpan] -> [[Alignable GHC.RealSrcSpan]]
groupAlign a xs =
  case a of
    Never    -> []
...
recordToAlignable conf =
  groupAlign (cRecords conf) . fromMaybe [] . traverse fieldDeclToAlignable

--------------------------------------------------------------------------------
fieldDeclToAlignable ::
     GHC.LocatedA (Hs.ConDeclField Hs.GhcPs)
...
matchGroupToAlignable conf (Hs.MG _ alts _) = cases' ++ patterns'
  where
    (cases, patterns) =
      partitionEithers . fromMaybe [] $
      traverse matchToAlignable (GHC.unLoc alts)
...
multiWayIfToAlignable ::
     Config -> Hs.LHsExpr Hs.GhcPs -> [[Alignable GHC.RealSrcSpan]]
...
step maxColumns config =
  makeStep "Cases" $ \ls module' ->
    let changes ::
        ...
        configured =
          ...
          changes everything (multiWayIfToAlignable config)
     in Editor.apply configured ls
