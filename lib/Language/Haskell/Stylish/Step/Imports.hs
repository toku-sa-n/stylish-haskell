...
formatGroup maxCols options moduleStats imports =
  let newLines = formatImports maxCols options moduleStats imports
   in Editor.changeLines (importBlock imports) (const newLines)
...
importBlock group =
  ...
  where
    src =
      fromMaybe (error "importBlock: missing location") .
...
formatImports maxCols options moduleStats rawGroup =
  runPrinter_
    ...
    do let group :: NonEmpty (GHC.LImportDecl GHC.GhcPs)
           group =
             NonEmpty.sortBy (compareImports `on` GHC.unLoc) rawGroup &
             mergeImports
           unLocatedGroup = GHC.unLoc <$> toList group
...
printQualified Options {..} padNames stats ldecl = do
  ...
  let module_ = do
        ...
        let somethingFollows =
              isJust (GHC.ideclAs decl) ||
              isHiding decl || not (null $ GHC.ideclHiding decl)
        when (padNames && somethingFollows) $
          putText $
  ...
  case snd <$> GHC.ideclHiding decl of
    Nothing -> pure ()
    Just limports
      | null (GHC.unLoc limports) ->
        case emptyListAlign of
    ...
    Just limports -> do
      let imports = GHC.unLoc limports
          printedImports =
            flagEnds $ -- [P ()]
            printImport separateLists . GHC.unLoc <$> prepareImportList imports
              ...
              wrapping
                ...
                (do case listAlign
                    ...
                    case listAlign
                      ...
                      _
                        | start -> putText "(" >> doSpaceSurround
                          -- Don't bother aligning if we're not in inline mode.
                      _
                        | longListAlign /= Inline -> pure ()
      ...
      case longListAlign of
        Multiline -> wrapping (space >> printAsSingleLine) printAsMultiLine
        Inline
          | NewLine <- listAlign -> do
            modifyCurrentLine trimRight
        ...
        InlineWithBreak ->
          wrapping
            ...
            (do modifyCurrentLine trimRight
  ...
  where
    ...
    patchForRepeatHiding =
      case listAlign of
        Repeat
          | isHiding decl -> withColumns Nothing
        _ -> id

--------------------------------------------------------------------------------
printImport :: Bool -> GHC.IE GHC.GhcPs -> P ()
printImport _ (GHC.IEVar _ name) = printIeWrappedName name
printImport _ (GHC.IEThingAbs _ name) = printIeWrappedName name
...
printImport separateLists (GHC.IEThingWith _ name wildcard imps) = do
  ...
  let ellipsis =
        case wildcard of
          ...
          GHC.NoIEWildcard         -> []
  parenthesize $ sep (comma >> space) (ellipsis <> fmap printIeWrappedName imps)
printImport _ (GHC.IEGroup {}) =
  error
...
printIeWrappedName lie =
  case GHC.unLoc lie of
...
mergeImports (x :| []) = x :| []
mergeImports (h :| (t:ts))
  | canMergeImport (GHC.unLoc h) (GHC.unLoc t) =
    mergeImports (mergeModuleImport h t :| ts)
  | otherwise = h :| mergeImportsTail (t : ts)
  where
    mergeImportsTail (x:y:ys)
      | canMergeImport (GHC.unLoc x) (GHC.unLoc y) =
        mergeImportsTail (mergeModuleImport x y : ys)
...
prepareImportList =
  ...
  where
    ...
    -- mess this up, though.  So they end up in the tail of the list.
        (\(x :| xs) (y :| ys) ->
           case ieMerge (GHC.unLoc x) (GHC.unLoc y) of
