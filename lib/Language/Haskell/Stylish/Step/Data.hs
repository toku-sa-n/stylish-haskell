...
formatDataDecl cfg@Config {..} decl@MkDataDecl {..} =
  ...
  where
    originalDeclBlock =
      Editor.Block (GHC.srcSpanStartLine dataLoc) (GHC.srcSpanEndLine dataLoc)
    printerConfig =
      PrinterConfig
        { columns =
            case cMaxColumns of
...
putDataDecl cfg@Config {..} decl = do
  ...
  when
    (hasConstructors decl)
    do case (cEquals, cFirstField) of
         (_, Indent x)
           | isEnum decl && cBreakEnums -> newline >> spaces x
         (_, _)
           | not (isNewtype decl)
           , singleConstructor decl && not cBreakSingleConstructors -> space
         (Indent x, _)
           | onelineEnum -> space
           | otherwise -> newline >> spaces x
         (SameLine, _) -> space
       lineLengthAfterEq <- fmap (+ 2) getCurrentLineLength
       if | onelineEnum -> putText "=" >> space >> putUnbrokenEnum cfg decl
          | isNewtype decl ->
            do putText "=" >> space
               forM_ (GHC.dd_cons defn) $ putNewtypeConstructor cfg
          | not . null $ GHC.dd_cons defn ->
            do forM_ (flagEnds constructorComments) $ \(CommentGroup {..}, firstGroup, lastGroup) -> do
                 forM_ cgPrior $ \lc -> do
                 ...
                 forM_ (flagEnds cgItems) $ \((lcon, mbInlineComment), firstItem, lastItem) -> do
                   ...
                   unless (lastGroup && lastItem) $ consIndent lineLengthAfterEq
  ...
  when
    (hasDeriving decl)
    do if onelineEnum && null derivingComments
         then space
         else do
           ...
           newline
           spaces cDeriving
       sep (newline >> spaces cDeriving) $
         map (putDeriving cfg) (GHC.dd_derivs defn)
  where
    consIndent eqIndent =
      newline >>
...
putDeriving Config {..} lclause = do
  let clause@GHC.HsDerivingClause {..} = GHC.unLoc lclause
      tys =
        (if cSortDeriving
           then sortBy compareOutputableCI
           else id) $
        map (GHC.sig_body . GHC.unLoc) $ derivingClauseTypes clause
      headTy = listToMaybe tys
      tailTy = drop 1 tys
  putText "deriving"
  forM_ deriv_clause_strategy $ \lstrat ->
    case GHC.unLoc lstrat of
  ...
  putCond
    ...
    do space
       putText "("
       sep (comma >> space) (fmap putOutputable tys)
       putText ")"
    do newline
       spaces indentation
       putText "("
       forM_ headTy \t -> space >> putOutputable t
       forM_
         tailTy
         \t -> do
           newline
...
putName decl@MkDataDecl {..} =
  ...
  where
    ...
    maybePutKindSig =
      forM_
        maybeKindSig
        (\k -> space >> putText "::" >> space >> putOutputable k)
...
putConstructor cfg consIndent lcons =
  case GHC.unLoc lcons of
    GHC.ConDeclGADT {..}
    ...
    -- Put type of constructor:
      ...
      case con_g_args of
        GHC.PrefixConGADT scaledTys ->
          forM_ scaledTys $ \scaledTy -> do
    ...
    GHC.ConDeclH98 {..} -> do
      ...
      case con_args of
        ...
        GHC.RecCon largs
          | _:_ <- GHC.unLoc largs -> do
            ...
            forM_ (flagEnds commented) $ \(CommentGroup {..}, firstCommentGroup, _)
      ...
      where skipToBrace =
              case (cEquals cfg, cFirstField cfg) of
                (_, Indent y)
                  | not (cBreakSingleConstructors cfg) -> newline >> spaces y
      ...
      -- Jump to the next declaration.
            sepDecl bracePos =
              ...
              spaces
                case (cEquals cfg, cFirstField cfg) of
                  (_, Indent y)
                    | not (cBreakSingleConstructors cfg) -> y
...
putNewtypeConstructor cfg lcons =
  case GHC.unLoc lcons of
    GHC.ConDeclH98 {..} ->
      putRdrName con_name >>
      case con_args of
...
putContext Config {..} lctx =
  suffix (space >> putText "=>" >> space) $
  case ltys of
    [lty]
      | GHC.HsParTy _ tp <- GHC.unLoc lty
      , cCurriedContext -> putType tp
    [ctx] -> putType ctx
    ctxs
      | cCurriedContext ->
        sep (space >> putText "=>" >> space) (fmap putType ctxs)
    ctxs -> parenthesize $ sep (comma >> space) (fmap putType ctxs)
  where
...
putConDeclField cfg GHC.ConDeclField {..} = do
  sep (comma >> space) (fmap putOutputable cd_fld_names)
  space
  putText "::"
  space
    ...
    GHC.HsForAllTy GHC.NoExtField tele tp -> do
      ...
      sep space $
        case tele of
          ...
          GHC.HsForAllInvis {..} ->
            putOutputable . GHC.unLoc <$> hsf_invis_bndrs
      case tele of
