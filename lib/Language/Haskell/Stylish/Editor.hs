...
replace line startCol endCol repl
  ...
  | otherwise = Edits $ M.singleton line [CLine startCol endCol repl]
...
--------------------------------------------------------------------------------
data Conflict =
  Conflict Int Change Int Change

--------------------------------------------------------------------------------
prettyConflict :: Conflict -> String
prettyConflict (Conflict l1 c1 l2 c2) =
  unlines
...
apply (Edits edits) =
  ...
  where
    ...
    go i (CBlock n f:_cs) ls =
      let (domain, ls') = splitAt n ls
       in f domain ++ go (i + n) (editsFor $ i + n) ls'
    go i (CLine xstart xend x:cs) (l:ls) =
      let l' = take (xstart - 1) l ++ x ++ drop (xend - 1) l
       in go i (adjust xstart xend x <$> cs) (l' : ls)
    adjust _ _ _ (CInsert xs) = CInsert xs
    adjust _ _ _ (CBlock n f) = CBlock n f
    adjust xstart xend x (CLine ystart yend y)
      | ystart >= xend =
        let offset = length x - (xend - xstart)
         in CLine (ystart + offset) (yend + offset) y
